import {
  getCollection, getSettings, sendTelegramMessage, editTelegramMessage, deleteTelegramMessage,
  toSmallCaps, getToken, getMainToken, esc, isMainBot, getCurrentBotId, formatISTDateTime,
  getISTDateString, sendTelegramFileBuffer, updateSettings, sendTelegramPhoto
} from '../bot-common.js';
import {
  getBotUsername, getDbChannelId, checkSubscription, isBotAdmin, extractChannelMessage,
  copyIntoDbChannel, copyFromDbChannel, getMainBotUsername, resolveUser, checkChannelsHealth,
  extractChannelMessageRange, getDbChannelReadinessError, getExportLinksKeyboard,
  getAdminDashboardKeyboard, getAdminHelpMessage, renderSystemStatus, renderPingReport,
  getUserHelpMessage, sendDatabaseBackup, getBackupDbChannelId
} from '../bot-helpers.js';
import {
  ...
  generateTempToken, getTempToken, revokeTempToken, listActiveTempTokens, parseDurationString,
  formatDuration, recordVerificationRedeemed, deleteStoredRecord, updateStoredRecordTitle,
...
async function handleStartCommand({ chatId, rawText, message, admin, sessions, isNewUser }) {
...
async function handleBundleCommand({ chatId, rawText, message }) {
  ...
  if (single) {
    ...
    await setBundleSession(chatId, {
      ...
      qualities: [],
      title: customTitle || '',
      sessionMsgId: promptMsg?.messageId
    });
    return;
  }
...
async function handleBatchCommand({ chatId }) {
  const dbChannelId = await getDbChannelId();
  if (!dbChannelId) {
    const mainBotUsername = await getMainBotUsername();
    const setLink = `https://t.me/${mainBotUsername}?start=setting`;
...
async function handleUserStatsCommand({ chatId }) {
...
async function handleTopFilesCommand({ chatId }) {
...
async function handleTodayLinksCommand({ chatId }) {
...
async function handleBulkStoreCommand({ chatId }) {
...
async function handleExportLinksCommand({ chatId, rawText }) {
...
async function handleBroadcastCommand({ chatId, rawText, message, sessions }) {
...
async function handleBanCommand({ chatId, rawText, message }) {
  let targetArg = null;
...
async function handleUnbanCommand({ chatId, rawText, message }) {
  const parts = rawText.trim().split(/\s+/).slice(1);
  const replyUser = message.reply_to_message?.from || message.reply_to_message?.forward_from;
  let targetArg = null;
...
async function handleBanListCommand({ chatId }) {
...
async function handleUserInfoCommand({ chatId, rawText, message }) {
  const parts = rawText.trim().split(/\s+/).slice(1);
  const replyUser = message.reply_to_message?.from || message.reply_to_message?.forward_from;
  ...
  const profileText = `👤 <b>User Profile Inspection</b>\n\n` +
    ...
    `• <b>Joined At:</b> <code>${formatISTDateTime(profile.joinedAt)}</code>\n` +
    `• <b>Last Seen:</b> <code>${formatISTDateTime(profile.lastSeen)}</code>\n` +
...
async function handleDeleteCommand({ chatId, rawText }) {
...
async function handleWipeCommand({ chatId }) {
...
async function handleEditFileCommand({ chatId, rawText }) {
...
async function handleCheckChannelsCommand({ chatId }) {
...
async function handleRebuildChannelCommand({ chatId, rawText }) {
  const parts = rawText.trim().split(/\s+/);
...
async function handleSettingCommand({ chatId }) {
...
async function handleAdminHelpCommand({ chatId }) {
...
async function handleAuditLinksCommand({ chatId }) {
  await renderStorageAudit(chatId);
}

async function handleScanBrokenCommand({ chatId, rawText }) {
  const parts = rawText.trim().split(/\s+/);
...
async function handleTempTokenCommand({ chatId, rawText, message }) {
  const parts = rawText.trim().split(/\s+/);
...
async function handleMyTokensCommand({ chatId, admin }) {
...
async function handleRevokeTokenCommand({ chatId, rawText, admin }) {
...
async function handleHelpCommand({ chatId, admin }) {
  const { text, replyMarkup } = getUserHelpMessage(admin);
  await sendTelegramMessage(chatId, text, replyMarkup);
}

// ─── Ordered Command Registry ─────────────────────────────────────────────────
// Each entry specifies pattern, role restrictions, and handler function.
// Evaluated in exact sequence to preserve command priority.

export const commandRegistry = [
  { name: 'start', pattern: /^\/start/i, handler: handleStartCommand },
  { name: 'bundle', pattern: /^\/(bundle|quality)/i, canGenerateOnly: true, handler: handleBundleCommand },
  { name: 'batch', pattern: /^\/batch/i, canGenerateOnly: true, handler: handleBatchCommand },
  { name: 'store', pattern: /^\/store/i, canGenerateOnly: true, handler: handleStoreCommand },
  { name: 'cancel', pattern: /^\/cancel/i, adminOnly: true, handler: handleCancelCommand },
  { name: 'userstats', pattern: /^\/userstats/i, adminOnly: true, handler: handleUserStatsCommand },
  { name: 'backup', pattern: /^\/(backup|exportdb)/i, adminOnly: true, handler: handleBackupCommand },
  { name: 'topfiles', pattern: /^\/topfiles/i, adminOnly: true, handler: handleTopFilesCommand },
  { name: 'todaylinks', pattern: /^\/todaylinks/i, adminOnly: true, handler: handleTodayLinksCommand },
  { name: 'bulkstore', pattern: /^\/bulkstore/i, adminOnly: true, handler: handleBulkStoreCommand },
  { name: 'exportlinks', pattern: /^\/exportlinks/i, adminOnly: true, handler: handleExportLinksCommand },
  { name: 'broadcast', pattern: /^\/broadcast\s+/i, adminOnly: true, handler: handleBroadcastCommand },
  { name: 'ban', pattern: /^\/ban(\s+|$)/i, adminOnly: true, handler: handleBanCommand },
  ...
  { name: 'editfile', pattern: /^\/(editfile|rename)\s+(\S+)\s+(.+)/i, adminOnly: true, handler: handleEditFileCommand },
  { name: 'checkchannels', pattern: /^\/checkchannels/i, adminOnly: true, handler: handleCheckChannelsCommand },
  { name: 'rebuildchannel', pattern: /^\/rebuildchannel/i, adminOnly: true, handler: handleRebuildChannelCommand },
  { name: 'me', pattern: /^\/me$/i, handler: handleMeCommand },
  { name: 'setting', pattern: /^\/setting/i, adminOnly: true, handler: handleSettingCommand },
  ...
  { name: 'mytokens', pattern: /^\/(mytokens|temptokens)/i, handler: handleMyTokensCommand },
  { name: 'revoketoken', pattern: /^\/revoketoken\s+(\S+)/i, handler: handleRevokeTokenCommand },
  { name: 'help', pattern: /^\/help/i, handler: handleHelpCommand }
];
...
export async function processMessageUpdate(chatId, rawText, message, admin, req) {
  ...
  const ctx = { chatId, rawText, message, admin, req, users, sessions, isNewUser, canGenerate };

  // ─── Dispatch via Ordered Command Registry ────────────────────────────────────
  for (const cmd of commandRegistry) {
    if (cmd.adminOnly && !admin) continue;
    if (cmd.canGenerateOnly && !canGenerate) continue;

    const match = cmd.pattern.exec(rawText);
    if (match) {
      await cmd.handler({ ...ctx, match });
      return;
    }
  }
